...
class HistoryActionTests extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "deletion of a non existing records" should "not throw error" in withEmptyHistory { emptyHistoryF =>
    ...
    } yield err.isLeft shouldBe false
  }
  ...
  "history" should "correctly build the same trees in different ways" in withEmptyHistory { emptyHistoryF =>
  ...
  "randomly insert or delete" should "return the correct result" in withEmptyHistory { emptyHistoryF =>
    ...
    for {
      ...
      _            <- (1 to 10).toList.foldLeftM[
                        ...
                        for {
                          ...
                          _          <- newState.toList.traverse { case (k, value) =>
                                          for {
