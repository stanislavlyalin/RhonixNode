...
class ReplayRSpace[F[_]: Async: Log: Metrics: Span, C, P, A, K](
  ...
  override protected[this] def lockedConsume(
    channels: Seq[C],
    patterns: Seq[P],
    continuation: K,
    persist: Boolean,
    ...
    Span[F].traceI("locked-consume") {
      for {
        ...
        r <- replayData
               .get(consumeRef)
               .fold(storeWaitingContinuation(channels, wk))(comms =>
                 getCommAndConsumeCandidates(channels, patterns, comms.iterator().asScala.toList)
                   .flatMap {
                     _.fold(storeWaitingContinuation(channels, wk)) { case (_, dataCandidates) =>
                       for {
  ...
  override protected[this] def lockedProduce(
    channel: C,
    data: A,
    persist: Boolean,
    produceRef: Produce,
        ...
        result          <- replayData.get(produceRef) match {
                             ...
                             case Some(comms) =>
                               getCommOrProduceCandidate(
                                 ...
                                 _.fold(storeData(channel, data, persist, produceRef)) { case (_, pc) =>
                                   handleMatch(pc, comms)
                                 },
                               )
                           }
      } yield result
        ...
        store
          ...
          .map(
            _.iterator.zipWithIndex
              .filter { case (WaitingContinuation(_, _, _, _, source), _) =>
                comm.consume == source
              }
              .toSeq,
          ),
  ...
  private def removeBindingsFor(
    ...
    commRef.produces.foldLeft(replayData.removeBinding(commRef.consume, commRef)) { case (updatedReplays, produceRef) =>
  ...
  override protected def logConsume(
    consumeRef: Consume,
    channels: Seq[C],
    patterns: Seq[P],
    continuation: K,
  ...
  override protected def logProduce(
    produceRef: Produce,
    channel: C,
    data: A,
    persist: Boolean,
