...
trait TwoStepLock[F[_], K] {
  def acquire[R, S, W](keysA: Seq[K])(phaseTwo: () => F[Seq[K]])(thunk: => F[W])(implicit
    o: Ordering[K],
  ): F[W]
}

class ConcurrentTwoStepLockF[F[_]: Async: Metrics, K](ms: Metrics.Source) extends TwoStepLock[F, K] {
  ...
  override def acquire[R, S, W](keysA: Seq[K])(phaseTwo: () => F[Seq[K]])(thunk: => F[W])(implicit
    o: Ordering[K],
  ): F[W] =
