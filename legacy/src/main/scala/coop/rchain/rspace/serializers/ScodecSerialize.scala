...
object ScodecSerialize {
  ...
  def encodeContinuations[P, K](konts: Seq[WaitingContinuation[P, K]])(implicit
    sp: Serialize[P],
    sk: Serialize[K],
  ): ByteVector = {
  ...
  def decodeContinuations[P, K](bytes: ByteVector)(implicit
    sp: Serialize[P],
    sk: Serialize[K],
  ...
  implicit def codecEvent: Codec[Event] =
    discriminated[Event]
      .by(uint2)
      .subcaseP(tag = 0) { case comm: COMM =>
        comm
      }(codecCOMM)
      .subcaseP(tag = 1) { case produce: Produce =>
        produce
      }(codecProduce)
      .subcaseP(tag = 2) { case consume: Consume =>
        consume
      }(codecConsume)
