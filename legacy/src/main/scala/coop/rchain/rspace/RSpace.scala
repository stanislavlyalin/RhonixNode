  ...
  override protected[this] def lockedConsume(
    ...
    Span[F].traceI("locked-consume") {
      for {
        ...
        result               <- options.fold(storeWaitingContinuation(channels, wk))(dataCandidates =>
                                  for {
...
object RSpace {
  ...
  /**
    ...
    */
  def createHistoryRepo[F[_]: Async: Parallel: Log: Span, C, P, A, K](store: RSpaceStore[F])(implicit
    sc: Serialize[C],
    sp: Serialize[P],
    sa: Serialize[A],
    sk: Serialize[K],
