...
object StateChangeMerger {
  ...
  def computeTrieActions[F[_]: Async, C, P, A, K](
    ...
    val consumeAndJoinActionsCompute: F[Vector[ConsumeAndJoinActions]] =
      changes.kontChanges.toVector.traverse { case (consumeChannels, ChannelChange(added, removed)) =>
    ...
    for {
      ...
      produceTrieActions     <- changes.datumsChanges.toVector.traverse { case (historyPointer, changes) =>
                                  for {
      ...
      joinsTrieActions       <- joinsChanges.toVector
                                  .traverse { case (historyPointer, changes) =>
                                    mkTrieAction(
                                      historyPointer,
