...
object HotStoreState {
  def apply[C, P, A, K](
    continuations: Map[Seq[C], Seq[WaitingContinuation[P, K]]] = Map.empty[Seq[C], Seq[WaitingContinuation[P, K]]],
    installedContinuations: Map[Seq[C], WaitingContinuation[P, K]] = Map.empty[Seq[C], WaitingContinuation[P, K]],
...
final private case class HistoryStoreCache[F[_], C, P, A, K](
...
private class InMemHotStore[F[_]: Async, C, P, A, K](
      ...
      result           <- hotStoreState.modify { state =>
                            state.continuations
                              ...
                              .map(
                                ...
                                continuations => (state, state.installedContinuations.get(channels) ++: continuations),
                              )
                              .getOrElse(
  ...
  def toMap: F[Map[Seq[C], Row[P, A, K]]] =
    for {
      ...
      mapped        = data.padZip(continuations).view.mapValues { case (d, k) =>
