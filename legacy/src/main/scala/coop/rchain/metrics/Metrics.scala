...
sealed abstract class MetricsInstances {
  def readerTMetrics[F[_], E](m: Metrics[F]): Metrics[ReaderT[F, E, *]] =
    new Metrics[ReaderT[F, E, *]] {
      override def incrementCounter(name: String, delta: Long)(implicit
        ev: Source,
      ): ReaderT[F, E, Unit] =
        ReaderT.liftF(m.incrementCounter(name, delta)(ev))

      override def incrementSampler(name: String, delta: Long)(implicit
        ev: Source,
      ): ReaderT[F, E, Unit] =
      ...
      override def incrementGauge(name: String, delta: Long)(implicit
        ev: Source,
      ): ReaderT[F, E, Unit] =
        ReaderT.liftF(m.incrementGauge(name, delta)(ev))

      override def decrementGauge(name: String, delta: Long)(implicit
        ev: Source,
      ): ReaderT[F, E, Unit] =
        ReaderT.liftF(m.decrementGauge(name, delta)(ev))

      override def record(name: String, value: Long, count: Long)(implicit
        ev: Source,
      ): ReaderT[F, E, Unit] = ReaderT.liftF(m.record(name, value)(ev))

      override def timer[A](name: String, block: ReaderT[F, E, A])(implicit
        ev: Source,
