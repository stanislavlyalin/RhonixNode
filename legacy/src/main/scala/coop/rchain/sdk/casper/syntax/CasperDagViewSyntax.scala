...
final class CasperDagViewOps[F[_], M, MId, S, SId](private val dagView: DagView[F, M, MId, S, SId]) extends AnyVal {
  ...
  /**
    ...
    */
  def invalidJustificationFollows(msg: M, bondedSenders: Set[SId])(implicit
    s: Applicative[F],
  ): F[Boolean] =
    msg.justifications.toList
      .traverse(dagView.loadMessage)
