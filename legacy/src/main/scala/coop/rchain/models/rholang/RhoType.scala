...
object RhoType {
  ...
  object RhoByteArray {
    def unapply(p: Par): Option[Array[Byte]] =
      p.singleExpr.collect { case Expr(GByteArray(bs)) =>
        bs.toByteArray
      }
  ...
  object RhoString {
    def unapply(p: Par): Option[String] =
      p.singleExpr.collect { case Expr(GString(bs)) =>
        bs
      }
  ...
  object RhoBoolean {
    ...
    def unapply(p: Par): Option[Boolean] =
      p.singleExpr.collect { case Expr(GBool(b)) =>
        b
      }
  }
  ...
  object RhoNumber {
    def unapply(p: Par): Option[Long] =
      p.singleExpr.collect { case Expr(GInt(v)) =>
        v
      }
  ...
  object RhoUri {
    def unapply(p: Par): Option[String] =
      p.singleExpr.collect { case Expr(GUri(s)) =>
        s
      }
  ...
  object RhoList {
    def unapply(p: Par): Option[List[Par]] =
      p.singleExpr.collect { case Expr(EListBody(EList(s, _, _, _))) =>
        s.toList
      }
  ...
  object RhoSet {
    def unapply(p: Par): Option[Set[Par]] =
      p.singleExpr.collect { case Expr(ESetBody(ParSet(s, _, _, _))) =>
        s.toSet
      }
  ...
  object RhoMap {
    def unapply(p: Par): Option[Map[Par, Par]] =
      p.singleExpr.collect { case Expr(EMapBody(ParMap(s, _, _, _))) =>
        s.toMap
      }
  ...
  object RhoDeployerId {
    def unapply(p: Par): Option[Array[Byte]] =
      p.singleUnforgeable().collect { case GUnforgeable(GDeployerIdBody(id)) =>
  ...
  object RhoDeployId {
    def unapply(p: Par): Option[Array[Byte]] =
      p.singleUnforgeable().collect { case GUnforgeable(GDeployIdBody(id)) =>
        id.sig.toByteArray
      }
  ...
  object RhoName {
    def unapply(p: Par): Option[GPrivate] =
      p.singleUnforgeable().collect { case GUnforgeable(GPrivateBody(gprivate)) =>
        gprivate
      }
  ...
  object RhoSysAuthToken {
    def unapply(p: Par): Option[GSysAuthToken] =
      p.singleUnforgeable().collect { case GUnforgeable(GSysAuthTokenBody(token)) =>
        token
      }
  ...
  object Extractor {
    ...
    implicit def Tuple2Extractor[A, B](implicit
      A: Extractor[A],
      B: Extractor[B],
    ): Extractor[(A, B)] {
