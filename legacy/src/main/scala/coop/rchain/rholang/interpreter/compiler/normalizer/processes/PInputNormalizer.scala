...
object PInputNormalizer {
  ...
  def normalize[F[_]: Sync](p: PInput, input: ProcVisitInputs)(implicit
    env: Map[String, Par],
  ): F[ProcVisitOutputs] = {
    if (p.listreceipt_.size() > 1) {
      ...
      if (receiptContainsComplexSource) {
        ...
        def processSources(
          ...
          sources.foldM((Vector.empty[ParN], input.freeMap)) { case ((vectorPar, knownFree), name) =>
            NameNormalizeMatcher
              .normalizeMatch[F](name, NameVisitInputs(input.boundMapChain, knownFree))
              .map { case NameVisitOutputs(par, knownFree) =>
                (
                  vectorPar :+ par,
                  knownFree,
                )
        ...
        def processPatterns(
          ...
          patterns.traverse { case (names, nameRemainder) =>
            names
              .foldM((Vector.empty[ParN], FreeMap.empty[VarSort])) { case ((vectorPar, knownFree), name) =>
                NameNormalizeMatcher
                  .normalizeMatch[F](
                    name,
        ...
        for {
          ...
          receiveBindsAndFreeMaps            <- ReceiveBindsSortMatcher.preSortBinds[F, VarSort](
                                                  processedPatterns.zip(sources).map { case ((a, b, c), e) =>
