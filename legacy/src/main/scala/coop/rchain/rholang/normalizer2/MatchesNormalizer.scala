...
object MatchesNormalizer {
  def normalizeMatch[F[_]: Apply: NormalizerRec, T: BoundVarWriter: FreeVarWriter](
    p: PMatches,
  ): F[EMatchesN] = {
    ...
    def normalizePattern(proc: Proc) = BoundVarWriter[T].withNewBoundVarScope(() =>
      FreeVarWriter[T].withNewFreeVarScope(() => NormalizerRec[F].normalize(proc)),
    )
