...
import coop.rchain.rholang.normalizer2.env.{BoundVarScope, BoundVarWriter, FreeVarScope, FreeVarWriter}
...
object MatchesNormalizer {
  def normalizeMatches[F[_]: Sync: NormalizerRec: BoundVarScope: FreeVarScope](
    ...
    def normalizePattern(proc: Proc) = BoundVarScope[F].withNewVarScope()(() => NormalizerRec[F].normalize(proc))
