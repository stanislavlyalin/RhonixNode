...
import weaver.data.{ConflictResolution, MergingData}
 ...
 */
final case class MeldState[M, T](
  txsMap: Map[M, Set[T]],
  ...
  def add(lazoId: M, m: MergingData[T], garbage: Set[M]): MeldState[M, T] = {
...
object MeldState {
  ...
  def empty[M, T]: MeldState[M, T] = MeldState(
    Map.empty[M, Set[T]],
    Map.empty[T, Set[T]],
    Map.empty[T, Set[T]],
    Set.empty[T],
  ...
  def resolve[M, T: Ordering](target: Set[M], meld: MeldState[M, T]): ConflictResolution[T] = {
  ...
  def computeRelationMaps[F[_]: Sync, T](
    ...
    txs.foldLeftM((Map.empty[T, Set[T]], Map.empty[T, Set[T]])) { case ((accCM, accDM), t) =>
      ...
      val dm = dependsMap(t, conflictSet diff unSeenTx).map(_ |+| accDM)
      (cm, dm).tupled
    }
  }
...
//  def tConflict[M, T](meld: Meld[M, T]) = Conflicts.conflictsForMap(meld.conflictsMap)
}
