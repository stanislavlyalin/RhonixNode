...
object GrpcServer {
  def apply[F[_]: Async](
    namespace: String,
    port: Int,
    services: List[ApiDefinition[F, Any, Any]],
  ): Resource[F, Server] =
    Dispatcher.sequential[F].flatMap { dispatcher =>
      val serviceDefinition = mkServerServiceDefinition(namespace, services, dispatcher)
      Resource.make(
        Sync[F].delay {
          NettyServerBuilder
            .forPort(port)
  ...
  private def mkMethodHandler[F[_], Req, Resp](
    definition: ApiDefinition[F, Req, Resp],
  ...
  private def mkServerServiceDefinition[F[_], A, B](
    namespace: String,
    definitions: List[ApiDefinition[F, A, B]],
    dispatcher: Dispatcher[F],
  ): ServerServiceDefinition =
    definitions
      .foldLeft(ServerServiceDefinition.builder(namespace)) { (builder, definition) =>
        builder.addMethod(
          GrpcMethod(definition),
          mkMethodHandler(definition, dispatcher),
        )
      }
      .build
}
